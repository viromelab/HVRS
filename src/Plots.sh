...
for tool in "${TOOLS[@]}" 
  ...
  cd cnt3  
  $(cat ../total_stats.tsv | tr ',' '.' | grep "${i}-" | awk '{if ($1=="DS9" || $1=="DS10" || $1=="DS11" || $1=="DS12" || $1=="DS13" || $1=="DS14" || $1=="DS15" || $1=="DS16") {print $0}}' > "${tool}")
  echo "$(sort -t$'\t' -k13 -n ${tool})" > ${tool}
  cd ..
  
  cd cnt0 
  $(cat ../total_stats.tsv | tr ',' '.' | grep "${i}-" | awk '{if ($1=="DS1" || $1=="DS2" || $1=="DS3" || $1=="DS4" || $1=="DS5" || $1=="DS6" || $1=="DS7" || $1=="DS8") {print $0}}' > "${tool}")
  echo "$(sort -t$'\t' -k13 -n ${tool})" > ${tool}
  cd ..
